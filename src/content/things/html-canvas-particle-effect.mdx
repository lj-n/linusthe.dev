...
Now we create a `class Pparticle`, which will be the template for our particle objects.
...
For this we update each particles radius `r` every loop.
...
> But wait, at my friends display the particles are animating a lot faster!
...
For consistency we wil add an `update` method to the `Particle` class:
...
The rectabgle's top-left corner is at `x`, `y`. Its size is specified by `width` and `height`.
...
At last we have to add the velocities to our particles position in its `update` method.
